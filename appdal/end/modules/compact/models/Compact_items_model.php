...
class Compact_items_model extends Purchase_model {
     ...
     * @return
     */
    public function get_compact_by_purchase($purchase_number)
    {
        $query_builder      = $this->purchase_db;
        $query_builder->select('compact_number')->where('purchase_number',$purchase_number);
        $results = $query_builder->order_by('id','desc')->get($this->table_name)->row_array();//获取最新一个合同号
        return $results;
    }
