...
class Purchase_shipping_report_loss_model extends Purchase_model
{
        ...
        if (!empty($sys_data)&&!empty($plan_data)) {
            ...
            foreach ($plan_data as $plan_demand) {
                ...
                if ($temp_data<=0) {
                    ...
                    $this->purchase_db->where('id', $plan_data['id'])->update('shipment_track_list', ['plan_qty'=>$plan_data['plan_qty']-$loss_amount,'push_to_wms'=>2]);
                ...
                $this->purchase_db->where('id', $plan_data['id'])->update('shipment_track_list', ['plan_qty'=>0,'push_to_wms'=>2]);
    ...
    public function get_report_loss_list($params,$offset=null,$limit=null,$page=1){
        ...
        if(isset($params['audit_status']) && is_numeric($params['audit_status'])){
            $this->purchase_db->where('pre.status',$params['audit_status']);
        }
     ...
     /*   echo $this->purchase_db->last_query();
       ...
       if (!empty($result)) {
           foreach ($result as $key => $value) {
               ...
               $value_temp['demand_status'] = !empty($demand_info)?$order_status_list[$demand_info['suggest_order_status']]:'';
               $value_temp['audit_status'] = isset($getReportlossApprovalStatus[$value['audit_status']])?$getReportlossApprovalStatus[$value['audit_status']]:'';
