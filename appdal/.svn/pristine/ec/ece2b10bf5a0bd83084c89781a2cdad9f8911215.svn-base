...
class ApplyReturnGoodsExport extends AbstractHugeExport
{
    ...
    {
        $this->_ci->load->classes('purchase/classes/ApplyReturnGoodsTemplate');
        $template = $this->_ci->ApplyReturnGoodsTemplate->get_default_template_cols();
        return $template;
    }
    ...
    public function set_translator() : AbstractHugeExport
    {
        ...
        $this->tran_func_map = [
            ...
            'processing_status' => 'getReturnProcessingStatus',
        ];
    ...
    public function run()
    {
        try
        {
            ...
            $file_name  = '入库后退货-申请明细列表_'.date('YmdHi');
            if ($this->_format_type == EXPORT_VIEW_NATIVE)
            {
                return false;
            }
            else
            {
                ...
                $trans = function ($row) use ($pick_cols, $col_map, $tran_func_map) {
                    $new     = [];
                    $time_cols    = ['create_time', 'end_time'];
//pr($row);exit;
