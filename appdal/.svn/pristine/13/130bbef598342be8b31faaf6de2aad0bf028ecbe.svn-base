 ...
 * Time: 15:01
 */
class ApplyReturnGoodsExportService
{
    /**
     * client
     *
    ...
    public function quick_export($gids = '', $profile = '*', $format_type = EXPORT_VIEW_PRETTY, $data_type = VIEW_BROWSER, $charset = 'UTF-8')
    {
        ...
        if ($gids != '')
        {
            ...
            $quick_sql = $clone_db->from($this->_ci->m_return_main->getTable())->where_in('id', $gids_arr)->order_by('create_time desc, id desc')->get_compiled_select('', false);
        }
        else
        {
        ...
        if ($this->_file_type == 'xlsx'){
            ...
            $this->data['data_list']['key'] = array_column($this->_ci->lang->myline('return_apply_detail_list'),'label');
            $this->data['data_list']['value'] = $this->_ci->m_return_main->query_quick_sql($quick_sql);
            $this->_ci->load->service('return/ApplyReturnGoodsListService');
            $this->data = $this->_ci->applyreturngoodslistservice->translate($this->data,true);
