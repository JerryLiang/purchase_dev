<?php
/**
 * 产品信息不全数据库模型类
 * User: Jaden
 * Date: 2019/01/03 17:23
 */

class Product_incomplete_model extends Api_base_model {

    protected $table_name   = 'product_incomplete';// 数据表名称

    public function __construct(){
        parent::__construct();
        $this->init();
        $this->setContentType('');
    }

    /**
    *获取符合条件的产品信息不全需求单
    */
    public function get_incomplete_list_all($params){
        $params['limit'] = !isset($params['limit']) || intval($params['limit']) <= 0 ?
            $this->_defaultPageSize :
            min(intval($params['limit']), $this->_maxPageSize);

        if (!isset($params['page']) || intval($params['page']) <= 0) {
            $params['page'] = 1;
        }
        // End

        // 2.调用接口
        $url = $this->_baseUrl . $this->_listUrl;
        $url .= '?' . http_build_query($params);
        $result = $this->httpRequest($url, '', 'GET');
        // 3.确认返回的数据是否与预期一样
        if (empty($result) || !isset($result['status'])) {
            $this->_errorMsg = "Request Error:" . json_encode($result);
            return null;
        }
        if (0==$result['status']) {
            $this->_errorMsg = $result['data_list'];
        }
        if (!$result['status']) {
            return null;
        }
        // End
        return $result;
    }


    /**
     * 产品信息不全导出
     * @author Jaden
     * 2019-1-16
     */
    public function export_product_incomplete_list($params){
        $url = $this->_baseUrl . $this->_exportUrl;
        $url .= '?' . http_build_query($params);
        $result = $this->httpRequest($url, '', 'GET');
        if (empty($result) || !isset($result['status'])) {
            $this->_errorMsg = "Request Error:" . json_encode($result);
            return null;
        }
        if (0==$result['status']) {
            $this->_errorMsg = $result['data_list'];
        }
        if (!$result['status']) {
            return null;
        }
        // End
        return $result;
    }


    /**
     * 产品信息不全推送
     * @author Jaden
     * 2019-1-16
     */
    public function push_product_incomplete_list($params){
        $url = $this->_baseUrl . $this->_purUrl;
        $url .= '?' . http_build_query($params);
        $result = $this->httpRequest($url, '', 'GET');
        if (empty($result) || !isset($result['status'])) {
            $this->_errorMsg = "Request Error:" . json_encode($result);
            return null;
        }
        if (0==$result['status']) {
            $this->_errorMsg = $result['errorMess'];
        }
        if (!$result['status']) {
            return null;
        }
        // End
        return $result;
    }

    /**
     * 添加备注
     * @author Jaden
     * 2019-3-14
     */

    public function web_create_remarks($params){
        $url = $this->_baseUrl . $this->_createremarksUrl;
        $url .= '?' . http_build_query($params);
        $result = $this->httpRequest($url, '', 'GET');
        // 3.确认返回的数据是否与预期一样
        if (empty($result) || !isset($result['status'])) {
            $this->_errorMsg = "Request Error:" . json_encode($result);
            return null;
        }
        if (0==$result['status']) {
            $this->_errorMsg = $result['errorMess'];
        }
        if (!$result['status']) {
            return null;
        }
        // End
        return $result;
    }


}