...
class CsvReader
{
     ...
     * 记入导入的内容
     */
    protected $save_original_line = false;

    protected $original_line = [];
    ...
    public function set_file_url($form_upload_name)
    {
        $this->form_upload_name = $form_upload_name;
    }
     ...
     * 原始的数据
     * @author Manson
     * @param bool $save_original_line
     * @return $this
     */
    public function set_save_original_line($save_original_line = false)
    {
        $this->save_original_line = $save_original_line;
        return $this;
    }

    public function set_error_file_url()
    {
        $this->dir_name = DIRECTORY_SEPARATOR.'download_csv'.DIRECTORY_SEPARATOR.'error_csv_'.date('Ymd').DIRECTORY_SEPARATOR;
        $this->reletive_path =  pathinfo($this->ci->upload->orig_name, PATHINFO_FILENAME).'_'.date('His').'_错误文件.csv';
        $this->dir_path = dirname(APPPATH).$this->dir_name;
        $this->file_path = $this->dir_path.$this->reletive_path;
        return $this;
    }
...
//pr($post);exit;
        ...
        //错误行内容
        $this->report['error_line_message'] = $result['data']['error_line_message']??[];

//        call_user_func_array($this->write_error, array($post));
        if ($num == 1){
            $this->write_error_csv($this->title);
        }else{
            $this->write_error_csv();
        }
    ...
    public function write_error_csv($title = '')
    {
        is_dir($this->dir_path) or mkdir($this->dir_path,0700);
        ...
        $this->report['errorLines'] = array_flip($this->report['errorLines']);

        foreach ($this->original_line as $key =>  $line) {
            if (isset($this->report['errorLines'][$key])){
                $line[] = '异常，可能是空行，请重新填写';
            }else{
                $line[] = '';
            }

            if (isset($this->report['error_line_message'][$key])){
                $line[] = $this->report['error_line_message'][$key];
            }else{
                $line[] = '';
            }
     ...
     */
    protected function valid_upload_check($file_url)
    {

        if (!empty($file_url)){
            $fhandle = fopen($file_url, 'r');
//                var_dump($fhandle);exit;
            if (!$fhandle)
            {
                $this->report['errorMess'] = '上传文件已经被删除';
                return false;
            }
            ...
            {
                $this->report['errorMess'] = $this->ci->upload->display_errors();
                return false;
            }
    ...
    public function set_general_insert_id(callable $cb,$not_id = false)
    {
    ...
    {
        if (!($fhandle = $this->valid_upload_check($this->form_upload_name)))
        {
//            var_dump($this->errorMess);exit;
            return false;
        }
        ...
        if (empty($title_line))
        {
            $this->report['errorMess'] = '无法检测处理上传文件格式编码，请转换为UTF-8编码或者GBK编码后上传';
            return false;
        }
...
//        $title = str_getcsv($title_line);
        ...
        array_walk($title, function(&$val) { $val = str_replace('""','"',trim(trim($val, " \t\n\r\0\x0B\""), " \t\n\r\0\x0B\"")); });
...
//        pr($this->required_cols);exit;
        ...
        while (($line = fgets($fhandle)) !== false)
        {
            ...
            $line = iconv($file_charset, "UTF-8//IGNORE", $line);

            $csv_row = $is_utf8 ? explode(",",trim($line)) : explode(",",trim($line)) ;
            array_walk($csv_row, function(&$val) { $val = trim(trim($val, " \t\n\r\0\x0B\""), " \t\n\r\0\x0B\""); });
            if ($this->save_original_line){
                $this->original_line[$this->csv_total_lines] = $csv_row;
            }
            //过滤掉纯空行
...
//pr($index_map_col);exit;
            ...
            else
            {
                if ($this->general_id_callback instanceof Closure)
                {
                    ...
                    if ($id === 'not_set_general_insert_id'){
                        $selected[] = $line;
                    }else{
                        //一个聚合可能有多个记录
                        $selected[$id][] = $line;
...
//pr($selected);exit;
    ...
    {
        if ($get_error_file) {
            $this->report['errorFiles'] = 'http://'.$_SERVER['HTTP_HOST'] . $this->dir_name . $this->reletive_path;
        }else{
     ...
     * @return string
     */
    protected function general_error_line_csv()
    {
        ...
        $dir_name = DIRECTORY_SEPARATOR.'download_csv'.DIRECTORY_SEPARATOR.'error_csv_'.date('Ymd').DIRECTORY_SEPARATOR;

        $reletive_path =  pathinfo($this->ci->upload->orig_name, PATHINFO_FILENAME).'_'.date('His').'_错误文件.csv';

        $dir_path = dirname(APPPATH).$dir_name;
        $file_path = $dir_path.$reletive_path;
        ...
        fclose($handle);
        return 'http://'.$_SERVER['HTTP_HOST'] . $dir_name . $reletive_path;
    }
    ...
    {
        if (empty($this->report['error_line_message'])) {
            return '';
        }


        $handle = fopen($this->form_upload_name, 'w+');
        if (!$handle) {
            $this->report['errorMess'] = '导出目录没有写入权限';
            return '';
        }
