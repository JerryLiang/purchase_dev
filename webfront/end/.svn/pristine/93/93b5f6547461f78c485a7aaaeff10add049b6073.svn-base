 ...
 *
 *          引入http_helper帮助类库
 *          $this->load->library('http_helper');
 *          1.请求
 *              get请求：
 *                  $responseData = $this->http_helper->sendRequest($requestUrl, $paramsData, 'GET', $curlOption = []);
 *              post请求：
 ...
 *              $this->http_helper->getHttpStatusCode();
 *          4.获取本次请求响应所有数据(可用作调试、记录日志)
 *              $this->http_helper->getSendRequestResult();
 *
 * Class Http_helper
 */
class Http_helper
{
    ...
    public static $CURL_OPTS = [
        ...
        CURLOPT_TIMEOUT         => 10000,//设置允许请求的最长时间(set maximum time the request is allowed to take)
