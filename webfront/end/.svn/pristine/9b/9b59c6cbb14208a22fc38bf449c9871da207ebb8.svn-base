 ...
 * Time: 11:01
 */
class Purchase_shipping_report_loss_model extends Api_base_model
{
   ...
   public  function export_report_loss($params){
        ...
        $objPHPExcel->setActiveSheetIndex(0)
            ...
            ->setCellValue('L1', '备货单状态')
            ->setCellValue('M1', '报损审核状态')
        ;
        ...
        if($result['status'] && !empty($result['data_list'])){
            ...
            foreach($data_list as $key => $val){
                ...
                $objPHPExcel->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('L'.$i, $val['demand_status'])
                    ->setCellValue('M'.$i, $val['audit_status'])
        ...
        //生成xlsx文件
        $filename = 'REPORT_LOSS_PLAN-'.date('YmdHis');
